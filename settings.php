...
if (!defined(CLI_SCRIPT) && $PAGE->url->compare(new moodle_url('/admin/settings.php', array('section'=>'authsettingdrupalservices')), URL_MATCH_PARAMS)) {
    ...
    if($remote_settings = (array)$drupalserver->Settings()){
      ...
      if(!$configempty && !empty($remote_settings['cookie_domain']) && (!$config->cookiedomain || $config->cookiedomain != $remote_settings['cookie_domain'])) {
        // the cookiedomain should get received via the Settings call
        $config->cookiedomain=$remote_settings['cookie_domain'];
...
//---------------------------- BEGIN ASLM-9 HACK-----------------------------------------------------------------------------------
//---------------------------------------------------------------------------------------------------------------------------------
          // If logged in as the anon user, log in as the cron sync_user user instead
          if ($loggedin_user->user->uid == 0) {
              $apiObj->status = 0;
              $remote_user = $config->remote_user;
              $remote_pw = $config->remote_pw;
              $loggedin_user = $apiObj->Login($remote_user, $remote_pw, true);
              if ($loggedin_user->info['http_code']==404) {
                  error_log("ERROR: Login service unreachable!\n");
              }
              if ($loggedin_user->info['http_code']==401) {
                  error_log("ERROR: Login failed - check username and password!\n");
              }
              elseif ($loggedin_user->info['http_code']!==200) {
                  $error = "ERROR: Login to drupal failed with http code " . $loggedin_user->info['http_code'];
                  if (!empty($loggedin_user->error)) {
                      $error .= PHP_EOL . $loggedin_user->error . PHP_EOL;
                  }
                  error_log($error);
              }
              $loggedin_user = $apiObj->Connect();
              $apiObj->status = 1;
          }
//---------------------------------------------------------------------------------------------------------------------------------
//---------------------------- END ASLM-9 HACK ------------------------------------------------------------------------------------
//---------------------------------------------------------------------------------------------------------------------------------
        ...
        if (!empty($loggedin_user->user->uid)) {
    ...
    if($config->cookiedomain !==false && $endpoint_reachable) {
      ...
      $drupalssosettings->add(new admin_setting_configcheckbox('auth_drupalservices/createusers',
              new lang_string('createusers', 'auth_drupalservices'),
              new lang_string('configcreateusers', 'auth_drupalservices'), 0));
      $drupalssosettings->add(new admin_setting_configtext('auth_drupalservices/pagesize',
              new lang_string('pagesize', 'auth_drupalservices'),
              new lang_string('configpagesize', 'auth_drupalservices'),
              20, PARAM_INT));
